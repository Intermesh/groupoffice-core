...
export class RecurrenceField extends Field {
	...
	constructor() {
		...
		this.picker.on('select', ({rule}) => {
			...
			this.value = rule;
			this.input!.value = RecurrenceField.toText(rule!, this.picker.startDate);
		});
	...
	protected internalSetValue(v?: any) {
		if(this.input) {
			this.input.value = RecurrenceField.toText(v, this.picker.startDate);
		}
	...
	protected createControl() {
		this.input = E('input').attr('type', 'text').attr('readOnly', true).cls('text');
		this.input.value = t('Not recurring'); // default state until changed
		return this.input;
	}
	setStartDate(date: DateTime) {
		this.picker.setStartDate(date);
		if(this.input)
			this.input.value = RecurrenceField.toText(this.value, this.picker.startDate);
	}
